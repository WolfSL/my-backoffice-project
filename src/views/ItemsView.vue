<template>
    ...
    <div class="container mx-auto flex flex-col h-full">
      ...
      <div class="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-lg flex flex-col flex-grow overflow-hidden">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-6 flex-shrink-0">
          <input v-model="filterCode" type="text" placeholder="Search Code..."
            class="px-2 py-1 dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300">
          <input v-model="filterDescription" type="text" placeholder="Search Description..."
            class="px-2 py-1 dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300">
          <select v-model="filterDepartment"
            class="px-2 py-1 dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300">
            <option value="">All Departments</option>
            <option v-for="dept in mastersStore.departments" :key="dept.DeptCode" :value="dept.Description">{{
              dept.Description }}</option>
          </select>
          <select v-model="filterSubDepartment"
            class="px-2 py-1 dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300">
            <option value="">All Sub-Departments</option>
            <option v-for="subDept in mastersStore.subDepartments" :key="subDept.SubDeptCode"
              :value="subDept.Description">{{ subDept.Description }}</option>
          </select>
          <select v-model="filterSupplier" class="px-2 py-1 dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300">
            <option value="">All Suppliers</option>
            <option v-for="supplier in mastersStore.suppliers" :key="supplier.id" :value="supplier.Descreption">{{
              supplier.Descreption }}</option>
          </select>
        </div>

        <div v-if="mastersStore.isLoading" class="text-center py-10 flex-grow">
          <p class="dark:text-gray-400">Loading items...</p>
        </div>
        <div v-else-if="mastersStore.error" class="text-center py-10 text-red-500 flex-grow">{{ mastersStore.error }}
        </div>
            ...
            <thead class="bg-gray-50 dark:bg-slate-700">
              <tr>
                ...
                <th
                  ...
                  Supplier</th>
                <th
                  class="sticky right-0 bg-gray-50 dark:bg-slate-700 px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                  Actions</th>
              </tr>
            </thead>
            <tbody class="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700">
              <tr v-for="item in filteredItems" :key="item.code">
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-200">{{
                  item.code }}</td>
                <td class="px-6 py-4 text-sm text-gray-800 dark:text-gray-300 max-w-xs whitespace-normal break-words">{{
                  item.description }}</td>
                ...
                <td
                  class="sticky right-0 bg-white dark:bg-slate-800 px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-4">
              ...
              <tr v-if="filteredItems.length === 0">
                <td colspan="6" class="text-center py-10 text-gray-500 dark:text-gray-400">No items match your criteria.
                </td>
              </tr>
            </tbody>
          </table>
  ...
  <div v-if="isModalOpen" class="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50">
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
      <div class="pl-5 pr-5 pt-2 pb-1 border-b dark:border-slate-700 flex justify-between items-center flex-shrink-0">
        <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-200">{{ isEditing ? 'Edit Product' : 'New Product' }}
        </h2>
        ...
        <div class="flex items-center space-x-2">
          ...
          <button v-if="isEditing" @click="printItemDetailsPDF"
            class="p-2 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-md">
          ...
          <button @click="isModalOpen = false"
            class="p-2 text-gray-400 rounded-full hover:bg-gray-200 dark:hover:bg-slate-700">&times;</button>
        </div>
      </div>

      <div class="p-6 flex-grow overflow-y-auto">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 mb-6 border-b dark:border-slate-700 pb-6">
          <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Department</label><select
              v-model="editableItem.department"
              class="mt-1 px-2 py-1 w-full dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300">
              <option v-for="d in mastersStore.departments" :key="d.DeptCode" :value="d.DeptCode">{{ d.Description }}
              </option>
            </select></div>
          <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Sub Department</label><select
              v-model="editableItem.subDepartment"
              class="mt-1 px-2 py-1 w-full dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300">
              <option v-for="sd in filteredSubDepartments" :key="sd.SubDeptCode" :value="sd.SubDeptCode">{{
                sd.Description }}</option>
            </select></div>
          <div>
            ...
            <div v-if="!isEditing" class="mt-1 relative rounded-md shadow-sm">
              <input v-model="editableItem.itemCode" type="text"
                class="block w-full pr-20 px-2 py-1 dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300">
              <button @click="getNextItemCode" type="button"
                class="absolute inset-y-0 right-0 px-4 py-2 border-l text-sm font-medium text-gray-600 hover:bg-gray-50 rounded-r-md">Get
                Next</button>
            </div>
            <input v-else :value="editableItem.itemCode" type="text" readonly
              class="mt-1 px-2 py-1 w-full bg-gray-100 dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300">
            <p v-if="validationErrors.itemCode" class="text-xs text-red-600 mt-1">{{ validationErrors.itemCode }}</p>
          </div>
          <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Unit</label><select
              v-model="editableItem.uom"
              class="mt-1 px-2 py-1 w-full dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300">
              <option v-for="u in mastersStore.units" :key="u.UnitCode" :value="u.UnitCode">{{ u.Description }}</option>
            </select></div>
          <div class="md:col-span-2">
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Name</label>
            <input v-model="editableItem.description" type="text"
              class="mt-1 px-2 py-1 w-full dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300">
            <p v-if="validationErrors.description" class="text-xs text-red-600 mt-1">{{ validationErrors.description }}
            </p>
          </div>
        </div>
        ...
        <div class="border-b border-gray-200 dark:border-slate-700">
          <nav class="-mb-px flex space-x-8" aria-label="Tabs">
            <button v-for="tab in tabs" :key="tab.name" @click="activeTab = tab.name"
              :class="[activeTab === tab.name ? 'border-indigo-500 text-indigo-600 dark:text-indigo-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 hover:border-gray-300']"
        ...
        <div class="pt-6">
          <div v-if="activeTab === 'Details'">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div class="space-y-4">
                <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Supplier</label><select
                    v-model="editableItem.supplierCode"
                    class="mt-1 px-2 py-1 w-full dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300">
                    <option v-for="s in mastersStore.suppliers" :key="s.id" :value="s.id">{{ s.Descreption }}</option>
                  </select></div>
                <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Cost</label><input
                    v-model.number="editableItem.listPrice" type="number"
                    class="mt-1 px-2 py-1 w-full dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300"></div>
                <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300">PLU Type</label><select
                    v-model="editableItem.pluType"
                    class="mt-1 px-2 py-1 w-full dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300">
                    <option v-for="pt in mastersStore.pluTypes" :key="pt.Code" :value="pt.Code">{{ pt.Description }}
                    </option>
                  </select></div>
                <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Printer</label><select
                    v-model="editableItem.itemType"
                    class="mt-1 px-2 py-1 w-full dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300">
                    <option v-for="p in mastersStore.printTypes" :key="p.PRNTTYPE" :value="p.PRNTTYPE">{{ p.TYPEDESC }}
                    </option>
                  </select></div>
              </div>
              <div class="space-y-4">
                <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Unitpack</label><input
                    v-model.number="editableItem.unitpack" type="number"
                    class="mt-1 px-2 py-1 w-full dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300"></div>
                <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Re-Order
                    Level</label><input v-model.number="editableItem.rol" type="number"
                    class="mt-1 px-2 py-1 w-full dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300"></div>
                <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Comments</label><input
                    v-model="editableItem.comments" type="text"
                    class="mt-1 px-2 py-1 w-full dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300"></div>
              </div>
              <div class="space-y-4 pt-2">
                <div v-for="option in itemOptions" :key="option.key" class="flex items-center">
                  <input v-model="editableItem[option.key]" :id="option.key" type="checkbox"
                    class="h-4 w-4 text-indigo-600 border-gray-300 rounded">
                  <label :for="option.key" class="ml-3 block text-sm font-medium text-gray-700 dark:text-gray-300">{{
                    option.label }}</label>
                </div>
              </div>
            </div>
            <div class="mt-6 border-t dark:border-slate-700 pt-6">
              <h3 class="text-lg font-semibold text-gray-700 dark:text-gray-300">Barcodes</h3><input
                v-model="editableItem.barcodes" type="text"
                class="mt-2 px-2 py-1 w-full md:w-1/2 dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300"
            ...
            <div class="flex flex-wrap items-end gap-4">
              <div class="relative flex-1 min-w-[240px]">
                ...
                <button @click="isLocationModalOpen = true"
                  class="mt-1 w-full text-left px-2 py-1 border border-gray-300 rounded-md shadow-sm bg-white dark:bg-slate-700 dark:border-slate-600">
                  {{ selectedLocations.length }} location(s) selected
                </button>
              </div>
              <div class="flex-1 min-w-[150px]">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Price Level</label>
                <select v-model="selectedPriceLevel"
                  class="mt-1 px-2 py-1 w-full dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300">
                  <option v-for="pl in mastersStore.priceLevels" :key="pl.PricelevelCode" :value="pl.PricelevelCode">{{
                    pl.PriceLevel }}</option>
                </select>
              </div>
              ...
              <div class="flex-1 min-w-[100px]">
                ...
                <input v-model.number="mSellingPrice" type="number"
                  class="mt-1 px-2 py-1 w-full dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300">
              </div>
              <div class="flex-shrink-0">
                <button @click="addPrice" type="button"
                  class="px-4 py-1 bg-indigo-600 text-white text-sm rounded-md hover:bg-indigo-700 flex items-center h-8">
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                  <thead class="bg-gray-50 dark:bg-slate-700">
                    <tr>
                      ...
                      <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">Price Level
                      </th>
                      <th class="px-4 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400">Selling</th>
                      <th class="px-4 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400">M.Selling
                      </th>
                  ...
                  <tbody class="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700">
                    <tr v-for="(price, index) in mastersStore.currentItemPrices" :key="index">
                      ...
                      <td class="px-4 py-2 text-sm text-right text-gray-800 dark:text-gray-300">{{
                        formatCurrency(price.selling) }}</td>
                      <td class="px-4 py-2 text-sm text-right text-gray-800 dark:text-gray-300">{{
                        formatCurrency(price.mSelling) }}</td>
                      <td class="px-4 py-2 text-center"><button @click="removePrice(index)"
                          class="text-red-500 hover:text-red-700">&times;</button></td>
                    </tr>
                    <tr v-if="mastersStore.currentItemPrices.length === 0">
                      <td colspan="5" class="text-center py-4 text-gray-500 dark:text-gray-400">No prices added yet.
                      </td>
                    </tr>
                  </tbody>
                </table>
          ...
          <div v-if="activeTab === 'BOM'">
            <div class="flex flex-wrap items-end gap-4">
              ...
              <div class="flex-shrink-0">
                <button @click="addBomComponent" type="button"
                  class="px-4 py-1 bg-indigo-600 text-white text-sm rounded-md hover:bg-indigo-700 flex items-center h-8">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd"
                      d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z"
            ...
            <div class="mt-6 border-t dark:border-slate-700 pt-4">
              <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-semibold text-gray-700 dark:text-gray-300">Bill of Materials for {{
                  selectedLocationName }}</h3>
              ...
              <div class="max-h-64 overflow-y-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                  <thead class="bg-gray-50 dark:bg-slate-700">
                    <tr>
                      <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">Item Code
                      </th>
                      <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">Description
                      </th>
                      ...
                      <th class="px-4 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400">Total Cost
                      </th>
                      <th class="px-4 py-2"></th>
                    </tr>
                  </thead>
                  <tbody class="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700">
                    <tr v-for="(bom, index) in mastersStore.currentItemBOM" :key="index">
                      ...
                      <td class="px-4 py-2 text-sm text-gray-500 dark:text-gray-400">{{ bom.unitpack }} {{ bom.uom }}
                      </td>
                      <td class="px-4 py-2 text-sm text-right text-gray-800 dark:text-gray-300">{{ bom.quantity }}</td>
                      <td class="px-4 py-2 text-sm text-right text-gray-800 dark:text-gray-300">{{
                        formatCurrency(bom.totalCost) }}</td>
                      <td class="px-4 py-2 text-center"><button @click="removeBomComponent(bom)"
                          class="text-red-500 hover:text-red-700">&times;</button></td>
                    </tr>
                    <tr v-if="mastersStore.currentItemBOM.length === 0">
                      <td colspan="6" class="text-center py-4 text-gray-500 dark:text-gray-400">No BOM components for
                        this location/price level.</td>
                    </tr>
                  </tbody>
                </table>
              </div>
      ...
      <div
        class="p-4 bg-gray-50 dark:bg-slate-900 border-t dark:border-slate-700 flex justify-end items-center space-x-4 flex-shrink-0">
        <button @click="isModalOpen = false" type="button"
          class="px-4 py-2 bg-white dark:bg-slate-700 text-gray-700 dark:text-gray-300 border dark:border-slate-600 rounded-md">Cancel</button>
        <button @click="handleSaveItem" :disabled="mastersStore.isAdding || mastersStore.isUpdating" type="submit"
          class="px-4 py-2 bg-indigo-600 text-white rounded-md disabled:bg-indigo-400">
          {{ isEditing ? (mastersStore.isUpdating ? 'Saving...' : 'Save Changes') : (mastersStore.isAdding ? 'Saving...'
            : 'Save Item') }}
        </button>
      </div>
    </div>
  ...
  <div v-if="isLocationModalOpen"
    ...
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-lg max-h-[70vh] flex flex-col">
      <div class="p-5 border-b dark:border-slate-700 flex-shrink-0">
        ...
        <input v-model="locationSearchText" type="text" placeholder="Search locations..."
        ...
        <ul>
          <li v-for="loc in filteredLocations" :key="loc.locCode" class="py-2 border-b dark:border-slate-700">
            <label class="flex items-center">
              <input type="checkbox" :value="loc.locCode" v-model="selectedLocations"
                class="h-4 w-4 text-indigo-600 border-gray-300 rounded">
              <div class="ml-3">
  ...
  <div v-if="isItemSelectorOpen" class="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50">
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-2xl max-h-[70vh] flex flex-col">
      ...
      <div class="p-6 overflow-y-auto flex-grow">
        <ul>
          <li v-for="item in mastersStore.transactionItems" :key="item.ItemCode" @click="selectBomItem(item)"
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400">{{ item.ItemCode }} - {{ item.SubDepartment }} | {{
              item.Unitpack }} {{ item.UOM }}</p>
          </li>
        </ul>
      </div>
      <div class="p-4 bg-gray-50 dark:bg-slate-900 border-t dark:border-slate-700 text-right flex-shrink-0">
  ...
  <div v-if="isDeleteModalOpen" class="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50">
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-md">
      <div class="p-6">
        <h3 class="text-lg font-medium text-gray-900 dark:text-gray-200">Delete Item</h3>
        <div class="mt-2">
      ...
      <div class="bg-gray-50 dark:bg-slate-900 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
        <button @click="handleDeleteItem" type="button"
        ...
        <button @click="isDeleteModalOpen = false" type="button"
          class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 dark:border-slate-600 shadow-sm px-4 py-2 bg-white dark:bg-slate-700 text-base font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-600 sm:mt-0 sm:w-auto sm:text-sm">Cancel</button>
      </div>
    </div>
  </div>
</template>
...
const { successMessage, operationError, totalBomCost } = storeToRefs(mastersStore);
...
const openEditModal = async (item) => {
  ...
  if (details) {
    ...
    // MODIFIED: Reset BOM state when opening edit modal
    mastersStore.currentItemBOM = [];
    bomLocation.value = '';
    bomPriceLevel.value = '';
...
const printItemDetailsPDF = async () => {
  ...
  console.log('Fetching prices and BOMs for item:', item.itemCode);
  const allPrices = await mastersStore.fetchItemPrices(item.itemCode, 'X', 'R');
  const allBOMs = await mastersStore.fetchItemBOM(item.itemCode, 'X', 'X', 'R');
  ...
  doc.autoTable({
    ...
    body: allPrices.map(p => [p.Locn_name, p.PriceLevel, formatCurrency(p.selling), formatCurrency(p.mSelling)]),
    theme: 'striped',
