<template>
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        <div class="md:col-span-2 bg-white dark:bg-slate-800 p-6 rounded-lg shadow-lg">
          ...
          <div class="flex items-center flex-wrap gap-4">
            <div class="relative flex-1 min-w-[200px]"><div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><svg class="h-5 w-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clip-rule="evenodd" /></svg></div><input type="date" id="dateFrom" v-model="dateFrom" @change="handleDataRefresh" class="block w-full pl-10 pr-3 py-2 border border-gray-300 dark:border-slate-600 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 bg-white dark:bg-slate-700 text-gray-600 dark:text-gray-300"></div>
            <div class="relative flex-1 min-w-[200px]"><div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><svg class="h-5 w-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clip-rule="evenodd" /></svg></div><input type="date" id="dateTo" v-model="dateTo" @change="handleDataRefresh" class="block w-full pl-10 pr-3 py-2 border border-gray-300 dark:border-slate-600 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 bg-white dark:bg-slate-700 text-gray-600 dark:text-gray-300"></div>
          </div>
        </div>
        <div class="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-lg flex flex-col justify-center">
          ...
          <p v-if="dashboardStore.isCusCountLoading" class="text-3xl font-bold text-gray-800 dark:text-gray-200 animate-pulse">...</p>
          <p v-else class="text-3xl font-bold text-gray-800 dark:text-gray-200">{{ dashboardStore.customerCount }}</p>
        </div>
        <div @click="isAiModalOpen = true" class="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-lg flex items-center cursor-pointer hover:shadow-xl transition-shadow">
          <div class="p-3 bg-purple-100 dark:bg-purple-900/50 rounded-full mr-4"><svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-purple-600 dark:text-purple-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" /></svg></div>
          <div><h3 class="text-gray-500 dark:text-gray-400 uppercase text-sm font-semibold">AI Insights</h3><p class="text-2xl font-bold text-gray-800 dark:text-gray-200">Analysis</p></div>
        </div>
      </div>

      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-lg flex flex-col">
          ...
          <div v-if="dashboardStore.isSalesSummaryLoading" class="space-y-2 flex-grow h-[450px] overflow-y-auto pr-2 animate-pulse"><div v-for="i in 6" :key="`ss-skel-${i}`" class="flex items-center p-2 rounded-lg bg-gray-200 dark:bg-slate-700 h-12"><div class="h-4 w-4 bg-gray-300 dark:bg-slate-600 rounded mr-3"></div><div class="flex-1 space-y-2"><div class="h-3 bg-gray-300 dark:bg-slate-600 rounded w-3/4"></div><div class="h-2 bg-gray-300 dark:bg-slate-600 rounded w-1/4"></div></div><div class="h-4 bg-gray-300 dark:bg-slate-600 rounded w-1/5"></div></div></div>
        ...
        <div class="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-lg flex flex-col">
          ...
          <div v-if="!dashboardStore.isSupplierSalesLoading && !dashboardStore.error" class="space-y-2 flex-grow h-[450px] overflow-y-auto pr-2"><div v-for="item in dashboardStore.supplierSales" :key="item.supName" class="p-3 rounded-lg bg-gray-50 dark:bg-slate-700/50 border border-gray-200 dark:border-slate-700"><div class="flex justify-between items-start"><p class="font-medium text-sm text-gray-800 dark:text-gray-200 flex-1 pr-4">{{ item.supName }}</p><p class="font-semibold text-base text-gray-900 dark:text-gray-100">{{ formatCurrency(item.amt) }}</p></div><div class="flex justify-between items-end mt-1 text-xs text-gray-500 dark:text-gray-400"><span>{{ item.pr.toFixed(2) }}% of Sales</span><span>{{ item.ltr.toLocaleString('en-US') }} ml</span></div></div></div>
        </div>
      </div>
      <div class="mt-6 bg-white dark:bg-slate-800 p-6 rounded-lg shadow-lg">
  ...
  <div v-if="isAiModalOpen" class="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50">
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
      <div class="p-5 border-b dark:border-slate-700 flex justify-between items-center"><h2 class="text-xl font-bold text-gray-800 dark:text-gray-200">AI Insights & Analysis</h2><button @click="isAiModalOpen = false" class="text-gray-400 hover:text-gray-600">&times;</button></div>
      <div class="p-6 flex-grow overflow-y-auto bg-gray-50 dark:bg-slate-900 space-y-6">
        <div class="bg-white dark:bg-slate-800 p-4 rounded-lg shadow"><div class="flex justify-between items-center mb-2"><h3 class="font-semibold text-gray-700 dark:text-gray-300">Sales Forecast</h3><button @click="dashboardStore.generateSalesForecast()" :disabled="dashboardStore.isForecastLoading" class="px-3 py-1 text-xs font-medium text-white bg-purple-600 rounded-md hover:bg-purple-700 disabled:bg-purple-400">Generate</button></div><div class="p-3 bg-gray-100 dark:bg-slate-700 rounded min-h-[60px]"><div v-if="dashboardStore.isForecastLoading" class="animate-pulse space-y-2"><div class="h-3 bg-gray-300 dark:bg-slate-600 rounded w-5/6"></div><div class="h-3 bg-gray-300 dark:bg-slate-600 rounded w-full"></div></div><p v-else class="text-sm text-gray-600 dark:text-gray-400 italic">{{ dashboardStore.aiForecast || 'Generate a forecast based on cash flow data.' }}</p></div></div>
        <div class="bg-white dark:bg-slate-800 p-4 rounded-lg shadow"><div class="flex justify-between items-center mb-2"><h3 class="font-semibold text-gray-700 dark:text-gray-300">Supplier Analysis</h3><button @click="dashboardStore.generateSupplierAnalysis()" :disabled="dashboardStore.isSupplierAnalysisLoading" class="px-3 py-1 text-xs font-medium text-white bg-purple-600 rounded-md hover:bg-purple-700 disabled:bg-purple-400">Generate</button></div><div class="p-3 bg-gray-100 dark:bg-slate-700 rounded min-h-[60px]"><div v-if="dashboardStore.isSupplierAnalysisLoading" class="animate-pulse space-y-2"><div class="h-3 bg-gray-300 dark:bg-slate-600 rounded w-5/6"></div><div class="h-3 bg-gray-300 dark:bg-slate-600 rounded w-full"></div></div><p v-else class="text-sm text-gray-600 dark:text-gray-400 italic">{{ dashboardStore.supplierAnalysis || 'Generate an analysis of supplier performance.' }}</p></div></div>
        <div class="bg-white dark:bg-slate-800 p-4 rounded-lg shadow"><div class="flex justify-between items-center mb-2"><h3 class="font-semibold text-gray-700 dark:text-gray-300">Peak Hours Analysis</h3><button @click="dashboardStore.generateHourlyAnalysis()" :disabled="dashboardStore.isHourlyAnalysisLoading" class="px-3 py-1 text-xs font-medium text-white bg-purple-600 rounded-md hover:bg-purple-700 disabled:bg-purple-400">Generate</button></div><div class="p-3 bg-gray-100 dark:bg-slate-700 rounded min-h-[60px]"><div v-if="dashboardStore.isHourlyAnalysisLoading" class="animate-pulse space-y-2"><div class="h-3 bg-gray-300 dark:bg-slate-600 rounded w-5/6"></div><div class="h-3 bg-gray-300 dark:bg-slate-600 rounded w-full"></div></div><p v-else class="text-sm text-gray-600 dark:text-gray-400 italic">{{ dashboardStore.hourlyAnalysis || 'Generate an analysis of peak business hours.' }}</p></div></div>
      </div>
      <div class="p-4 bg-white dark:bg-slate-800 border-t dark:border-slate-700 text-right"><button @click="isAiModalOpen = false" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">Close</button></div>
    </div>
  </div>

  <div v-if="dashboardStore.isXReportModalOpen" class="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50">
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-lg max-h-[80vh] flex flex-col"><div class="p-5 border-b dark:border-slate-700 flex justify-between items-center"><h2 class="text-xl font-bold text-gray-800 dark:text-gray-200">X-Report Details</h2><button @click="dashboardStore.closeXReportModal()" class="text-gray-400 hover:text-gray-600">&times;</button></div><div class="p-6 overflow-y-auto"><div v-if="dashboardStore.isXReportLoading" class="text-center py-10">Loading report...</div><table v-else class="min-w-full divide-y divide-gray-200 dark:divide-slate-700"><thead class="bg-gray-50 dark:bg-slate-700"><tr><th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Description</th><th class="px-4 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Amount</th></tr></thead><tbody class="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700"><tr v-for="reportItem in dashboardStore.xReportData" :key="reportItem.ID"><td class="px-4 py-2 whitespace-nowrap text-sm text-gray-800 dark:text-gray-300">{{ reportItem.DESCRIPTION }}</td><td class="px-4 py-2 whitespace-nowrap text-sm text-right text-gray-600 dark:text-gray-400">{{ formatCurrency(reportItem.AMOUNT) }}</td></tr></tbody></table></div><div class="p-4 bg-gray-50 dark:bg-slate-900 border-t dark:border-slate-700 text-right"><button @click="dashboardStore.closeXReportModal()" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">Close</button></div></div>
  </div>
</template>
